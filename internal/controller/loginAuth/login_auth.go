...
func (c *Auth) Login() {
	...
	BfToken = gtoken.GfToken{
		ServerName: "_gf_web",
		// Timeout:         g.gerror.New().MustGet(ctx, "gToken.Timeout").Int(), // 超时时间
...
func (c *Auth) loginFunc(r *ghttp.Request) (username string, data interface{}) {
	...
	if commonFunc.EncryptPassword(req.Password) != newData.Password || req.Username != newData.Username {
